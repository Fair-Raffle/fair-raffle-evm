...
library PerlinNoise {
  ...
  function noise2d(int256 x, int256 y) public pure returns (int256) {
    ...
    int256 d = lerp(
      u,
      grad2(a & 0xff, x, y - 0x10000),
      grad2(b & 0xff, x - 0x10000, y - 0x10000)
    );
  ...
  function noise3d(int256 x, int256 y, int256 z) public pure returns (int256) {
    ...
    a = lerp(
      u,
      grad3(scratch[3] >> 8, x, y, z),
      grad3(scratch[5] >> 8, x - 0x10000, y, z)
    );
    b = lerp(
      u,
      grad3(scratch[4] >> 8, x, y - 0x10000, z),
      grad3(scratch[6] >> 8, x - 0x10000, y - 0x10000, z)
    );
    c = lerp(v, a, b);

    a = lerp(
      u,
      grad3(scratch[3] & 0xff, x, y, z - 0x10000),
      grad3(scratch[5] & 0xff, x - 0x10000, y, z - 0x10000)
    );
    b = lerp(
      u,
      grad3(scratch[4] & 0xff, x, y - 0x10000, z - 0x10000),
      grad3(scratch[6] & 0xff, x - 0x10000, y - 0x10000, z - 0x10000)
    );
